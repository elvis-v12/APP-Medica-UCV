...
try {
    ...
    // Consulta SQL para obtener el historial completo de alertas del usuario ordenado por fecha descendente
    $sql = "SELECT ha.id_historial, ha.fecha, ha.estado, a.id_alerta, a.ubicacion, a.descripcion
            ...
            WHERE a.id_usuario = :id_usuario AND ha.estado = 'Atendido'
    ...
    // Consulta SQL para obtener el historial de alertas canceladas del usuario ordenado por fecha descendente
    $sql_canceladas = "SELECT ha.id_historial, ha.fecha, ha.estado, a.id_alerta, a.ubicacion, a.descripcion
                       FROM historialalertas ha 
                       INNER JOIN alertas a ON ha.id_alerta = a.id_alerta 
                       WHERE a.id_usuario = :id_usuario AND ha.estado = 'Cancelado'
                       ORDER BY ha.fecha DESC"; // Ordenar por fecha descendente

    // Preparar la consulta de alertas canceladas
    $stmt_canceladas = $db->prepare($sql_canceladas);
    $stmt_canceladas->bindParam(':id_usuario', $id_usuario, PDO::PARAM_INT);
    ...
    if ($stmt->rowCount() > 0) {
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            if ($fechaFormateada != $ultimaFecha) {
                ...
                echo "<thead><tr><th>Hora</th><th class='text-center'>Estado</th><th class='text-center'>Acción</th></tr></thead>";
                echo "<tbody>";
            }
            ...
            echo "<td class='text-center'><button class='btn btn-primary ver-alerta' data-id-alerta='" . $id_alerta . "' data-bs-toggle='modal' data-bs-target='#alertaModal'>Ver</button></td>";
            echo "</tr>";
        }
        ...
        echo "No hay registros de alertas.";
    }

    // Comprobar si se encontraron registros de alertas canceladas
    if ($stmt_canceladas->rowCount() > 0) {
        // Mostrar las alertas canceladas en una tabla
    echo "<div class='table-responsive'>";
    echo "<h5 class='mt-4'>Alertas Canceladas</h5>";
    echo "<table class='table table-striped'>";
    echo "<thead><tr><th>Fecha</th><th>Estado</th><th>Acción</th></tr></thead>";
    echo "<tbody>";
    while ($row = $stmt_canceladas->fetch(PDO::FETCH_ASSOC)) {
        $estado = htmlspecialchars($row["estado"]);
        $id_alerta = htmlspecialchars($row["id_alerta"]);
    ...
    // Imprimir cada alerta cancelada como una fila en la tabla
    echo "<tr>";
    ...
    echo "</div>";
        ...
        echo "<p>No hay registros de alertas canceladas.</p>";
    }
} catch (Exception $e) {
