...
try {
    ...
    // ===================== LISTADO POR DEFECTO =====================
    $sql = "SELECT u.id_usuario, u.nombre, u.apellido_paterno, u.apellido_materno, u.correo_institucional, r.nombre AS rol_nombre, ta.nombre AS tipo_acceso FROM usuarios u INNER JOIN roles r ON u.id_rol = r.id_rol LEFT JOIN registroacceso ec ON u.id_usuario = ec.id_usuario LEFT JOIN tipoacceso ta ON ec.id_tipo = ta.id_tipo ORDER BY u.id_usuario";
    $stmt = $db->prepare($sql);
    $stmt->execute();
    if ($stmt->rowCount() > 0) {
        echo "<table class='table table-striped'><thead><tr><th>ID</th><th>Nombres</th><th>Apellidos</th><th>Correo</th><th>Rol</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>";
